...
import type { SealRule } from './SealRule';
import {
    SealRuleFromJSON,
    SealRuleFromJSONTyped,
    SealRuleToJSON,
} from './SealRule';

/**
 * Seal access control policy for a deal
 * @export
 * @interface SealPolicy
 */
export interface SealPolicy {
    /**
     * Unique identifier for the Seal policy, often a Sui object ID.
     * @type {string}
     * @memberof SealPolicy
     */
    policyId: string;
     ...
     * @memberof SealPolicy
     */
    rules: Array<SealRule>;
}

/**
 * Check if a given object implements the SealPolicy interface.
 */
export function instanceOfSealPolicy(value: object): boolean {
    let isInstance = true;
...
export function SealPolicyFromJSON(json: any): SealPolicy {
    return SealPolicyFromJSONTyped(json, false);
}

export function SealPolicyFromJSONTyped(json: any, ignoreDiscriminator: boolean): SealPolicy {
    ...
    return {
        ...
        'rules': ((json['rules'] as Array<any>).map(SealRuleFromJSON)),
    };
}

export function SealPolicyToJSON(value?: SealPolicy | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
    ...
    return {
        ...
        'rules': ((value.rules as Array<any>).map(SealRuleToJSON)),
    };
}
