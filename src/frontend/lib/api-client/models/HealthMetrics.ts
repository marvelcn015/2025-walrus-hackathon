...
import type { HealthMetricsRisksDetectedInner } from './HealthMetricsRisksDetectedInner';
import {
    HealthMetricsRisksDetectedInnerFromJSON,
    HealthMetricsRisksDetectedInnerFromJSONTyped,
    HealthMetricsRisksDetectedInnerToJSON,
} from './HealthMetricsRisksDetectedInner';

/**
 * Overall deal health metrics
 * @export
...
export interface HealthMetrics {
    ...
    nextDeadline?: Date;
    /**
     * Average data completeness across all periods
     * @type {number}
     * @memberof HealthMetrics
     */
     ...
     * @type {Array<HealthMetricsRisksDetectedInner>}
     * @memberof HealthMetrics
     */
    risksDetected?: Array<HealthMetricsRisksDetectedInner>;
}

/**
 * Check if a given object implements the HealthMetrics interface.
 */
export function instanceOfHealthMetrics(value: object): boolean {
    let isInstance = true;
    isInstance = isInstance && "overallProgress" in value;
    isInstance = isInstance && "pendingActions" in value;
...
export function HealthMetricsFromJSONTyped(json: any, ignoreDiscriminator: boolean): HealthMetrics {
    ...
    return {
        ...
        'nextDeadline': !exists(json, 'nextDeadline') ? undefined : (new Date(json['nextDeadline'])),
        'dataCompletenessScore': !exists(json, 'dataCompletenessScore') ? undefined : json['dataCompletenessScore'],
        'risksDetected': !exists(json, 'risksDetected') ? undefined : ((json['risksDetected'] as Array<any>).map(HealthMetricsRisksDetectedInnerFromJSON)),
    };
}

export function HealthMetricsToJSON(value?: HealthMetrics | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
    ...
    return {
        ...
        'nextDeadline': value.nextDeadline === undefined ? undefined : (value.nextDeadline.toISOString().substring(0,10)),
        'dataCompletenessScore': value.dataCompletenessScore,
        'risksDetected': value.risksDetected === undefined ? undefined : ((value.risksDetected as Array<any>).map(HealthMetricsRisksDetectedInnerToJSON)),
    };
}
